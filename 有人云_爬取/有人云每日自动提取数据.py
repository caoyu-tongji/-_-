...
def run_data_extract():
    ...
    try:
        ...
        script_path = os.path.join(os.path.dirname(__file__), '有人云数据提取.py')
...
def main():
    # 设置定时任务：每天23:30运行数据提取
    schedule.every().day.at("23:30").do(run_data_extract)
    logging.info("已设置每天23:30运行数据提取任务")
    
    # 设置定时任务：每天23:59运行数据拼接
    schedule.every().day.at("23:59").do(run_data_merge)
    logging.info("已设置每天23:59运行数据拼接任务")
